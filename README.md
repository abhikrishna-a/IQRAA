...
CREATE INDEX idx_applications_internship_created ON applications (internship_id, created_at DESC);
...
SELECT id, student_id, cover_letter, status, resume, created_at FROM applications WHERE internship_id = 100 ORDER BY created_at DESC LIMIT 20 OFFSET 0;
...
Application.objects.filter(internship_id=100).only('id', 'student_id', 'cover_letter', 'status', 'resume', 'created_at').order_by('-created_at')[:20]
...
EXPLAIN (ANALYZE, BUFFERS) SELECT id, student_id, status, created_at FROM applications WHERE internship_id = 100 ORDER BY created_at DESC LIMIT 20;
